...
class ColorCLAHE:
    ...
    def apply(self, img: np.ndarray) -> np.ndarray:
        ...
        # Aplicar el filtro gaussiano a la imagen completa
        img_filtered = cv2.GaussianBlur(img_clahe, (5, 5), 0)
...
def plot_images(original: np.ndarray, processed: np.ndarray, color: bool = True, img_info: tuple = None):
    ...
    description = f"Bins: 256, Clip Limit: {clip_limit}, Tile Grid Size: {tile_grid_size}"
